...
class Prompt(QWidget):
    ...
    def __initUi(self):
        ...
        # Create the actions
        beginningAction = QAction(
            LangClass.TRANSLATIONS["Show Beginning"]
            + f" ({DEFAULT_SHORTCUT_PROMPT_BEGINNING})",
            self,
        )
        ...
        endingAction = QAction(
            LangClass.TRANSLATIONS["Show Ending"]
            + f" ({DEFAULT_SHORTCUT_PROMPT_ENDING})",
            self,
        )
        ...
        supportPromptCommandAction = QAction(
            LangClass.TRANSLATIONS["Support Prompt Command"]
            + f" ({DEFAULT_SHORTCUT_SUPPORT_PROMPT_COMMAND})",
            self,
        )
                ...
                if starts_with_f:
                    ...
                    if command_word:
                        filtered_commands = [
                            ...
                            if command_word.lower() in command.lower()
                        ]
    ...
    def record(self, checked):
        if checked:
            ...
            self.recorder_thread.errorGenerated.connect(self.on_recording_error)
