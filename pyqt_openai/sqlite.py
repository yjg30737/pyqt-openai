...
from pyqt_openai.models import ImageChatContainer, FavoriteChatContainer
...
class SqliteDatabase:
    ...
    def __initVal(self, db_filename):
        ...
        # etc table
        self.__favorite_table_nm = 'favorite_tb'
    ...
    def __createImage(self):
        try:
            ...
            if self.__c.fetchone()[0] == 1:
                ...
                required_columns = set(ImageChatContainer.get_keys_for_insert())
    ...
    def insertImage(self, arg: ImageChatContainer):
        try:
    ...
    def __createEtc(self):
        try:
            self.__createFavorite()
        except sqlite3.Error as e:
            print(f"An error occurred while creating the table: {e}")
            raise

    def __createFavorite(self):
        # Check if the table exists
        self.__c.execute(f"SELECT count(*) FROM sqlite_master WHERE type='table' AND name='{self.__favorite_table_nm}'")
        if self.__c.fetchone()[0] == 1:
            # To not make table every time to change column's name and type
            self.__c.execute(f'PRAGMA table_info({self.__favorite_table_nm})')
            self.__conn.commit()
        else:
            self.__c.execute(f'''CREATE TABLE {self.__favorite_table_nm}
            (id INTEGER PRIMARY KEY,
             id_fk INTEGER,
             model VARCHAR(255),
             ...
             update_dt DATETIME DEFAULT CURRENT_TIMESTAMP,
             insert_dt DATETIME DEFAULT CURRENT_TIMESTAMP,
             favorite_dt DATETIME DEFAULT CURRENT_TIMESTAMP)''')
            # Commit the transaction
            self.__conn.commit()
